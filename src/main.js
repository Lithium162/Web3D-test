...
function planetRevolution(){
    ...
    moonOrbit.rotation.y +=0.20944*planetData.speed;
...
// 3. 鼠标移动回调：基于 movementX/Y 改变飞船旋转
  function onPointerMove(event) {
    ...
    if (spaceship) {
      spaceship.rotation.y -= movementX * rotateSpeed;
      spaceship.rotation.x -= movementY * rotateSpeed;
    }
  }

// 4. 按 ESC 键退出指针锁（浏览器会自动执行，也可手动调用）
